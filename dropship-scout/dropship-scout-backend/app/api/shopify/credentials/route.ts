...
// Saves domain + token (upsert — replaces if already exists)
export async function POST(req: NextRequest) {
  const { userId, domain, token } = await req.json()
  if (!userId || !domain || !token) {
    return NextResponse.json({ error: 'userId, domain, and token are required' }, { status: 400 })
  }
  ...
  const { error } = await supabaseAdmin
    ...
    .upsert({
      ...
      shopify_token_enc: token, // In production: encrypt this before storing
    })
